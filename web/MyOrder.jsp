...
<!--                                        <a style="text-align: center;margin-top: 10px;" href="MyAccount.jsp" class="btn btn-outline-warning">View Profile</a>-->
                                ...
                                String[] part=null;
                                if (session!=null) {
                                       vString = (String)session.getAttribute("username");
                                        part = vString.split(":");
                                    }
                                    ...
                                    <table class="table table-hover table-light" style="margin-top: 10px;">
                                            <thead>
                                              <tr>
                                                ...
                                                <th scope="col" id="col_id">Product Id</th>
                                                <th scope="col" id="col_id">Paid Via</th>
                                                <th scope="col" id="col_id">Date</th>
                                                <th scope="col" id="col_id">status</th>
                                                <th scope="col" id="col_id">View Details</th>
                                ...
                                <sql:query var="rs" dataSource="${db}">select * from invoice_info where customer_ip='<%=ipp %>' and customer_id = (select customer_id from customer where customer_email = '<%=part[1] %>');</sql:query>
                                <c:forEach items="${rs.rows}" var="row">
                                    <tr>
                                       <th scope="row" id="col_id">${row.id}</th>
                                        <th id="col_id">${row.product_names}</th>
                                        <th id="col_id">${row.paid_via}</th>
                                        <th id="col_id">${row.date}</th>
                                        <th id="col_id">${row.status}</th>
                                        <th id="col_id">
                                            <a href='ViewOrderProduct.jsp?order_vid=${row.id}' class="btn btn-light">View</a>          
                                        </th>
                                        <th id="col_id">
                                            <form method="get" action="DeleteOrderUData">
