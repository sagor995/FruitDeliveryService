                                            ...
                                            <table class="table table-hover table-active">
                                                       <thead>
                                                         <tr>
                                                           ...
                                                           <th scope="col" id="col_id">Question</th>
                                                           <th scope="col" id="col_id">Answer</th>
                                                       ...
                                                       <tbody>
                                                            <br><br>
                                                            <center><a href="insert_eFAQ.jsp" class=" btn btn-outline-dark" ><b>Insert a new Frequently Asked Question?</b></a> </center>
                                                           <br>
                                                           <sql:setDataSource var="db" driver="com.mysql.jdbc.Driver" url="jdbc:mysql://localhost:3306/fruit_ds" user="root" password="" />
                                                           <sql:query var="rs" dataSource="${db}">select * from faq;</sql:query>
                                                           <c:forEach items="${rs.rows}" var="row">
                                                               <tr>
                                                                  <th scope="row" id="col_id">${row.id}</th>
                                                                   <th id="col_id">${row.question}</th>
                                                                   <th id="col_id">${row.answer}</th>
                                                                   <th id="col_id"><a href="edit_faq.jsp?edit_faq_id=${row.id}" class="btn btn-outline-light">Edit</a></th>
                                                                   <th id="col_id">
                                                                       <form method="get" action="../DeleteFaqData">
                                                                       <input type="hidden" name="delete_faq_id" value="${row.id}">
